  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        {/* 价值观总结 */}
        <div className="mt-10 rounded-2xl border border-border bg-card/60 p-8 backdrop-blur">
          <div className="grid gap-5 sm:grid-cols-3">
            {summary.map((s) => (
              <p key={s.k} className="text-sm leading-relaxed text-muted-foreground">
                <span className="font-bold text-foreground">{s.k}</span>
                {"，"}
                {s.v}
              </p>
            ))}
